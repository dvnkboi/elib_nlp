...
def group_by_dist(rectArr: List[Rect], minDist=10, removeRedundancies=True):
    ...
    if removeRedundancies:
        for pivotIdx, pivot in enumerate(groupRect):
            for rectIdx, rect in enumerate(
                groupRect[(pivotIdx + 1) :], start=(pivotIdx + 1)
            ):
...
def get_image_zones(image, mode="cv2_contours", options=defaults):
    ...
    # cv2.imshow("gray", filteredImage)
    # cv2.waitKey(0)
    if mode == "cv2_contours":
        filteredImage = ImUtils.filter_lines(filteredImage, (255, 255, 255))
        # cv2.imshow("lines", filteredImage)
        # cv2.waitKey(0)
        filteredImage = cv2.GaussianBlur(filteredImage, options["gaus_filt"], 0)
        # cv2.imshow("gaussian", filteredImage)
        # cv2.waitKey(0)
    filteredImage = ImUtils.thresholding(filteredImage, 0, 255)

    # cv2.imshow("threshold", filteredImage)
    # cv2.waitKey(0)
        ...
        for i in range(len(tessData["text"])):
            if int(tessData["conf"][i]) > options["tess_confid"]:
                if re.match(options["tess_pattern"], tessData["text"][i]):
                    ...
                    rectangles.append(
                        Rect(
                            x - options["rect_pad"][0],
                            y - options["rect_pad"][1],
                            w + options["rect_pad"][0],
                            h + options["rect_pad"][1],
                        )
                    )
    elif mode == "cv2_contours":
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, options["cont_smooth"])
    ...
    # cv2.imshow("contours", filteredImage)
    # cv2.waitKey(0)
...
def remove_lines(rectArr, minThresh=5, maxThresh=20) -> List[Rect]:
    for _, pivot in enumerate(rectArr):
        if (
            pivot.maxX - pivot.minX < minThresh and pivot.maxY - pivot.minY > maxThresh
        ) or (
            pivot.maxY - pivot.minY < minThresh and pivot.maxX - pivot.minX > maxThresh
        ):
            rectArr.remove(pivot)
    return rectArr


#! tesseract OCR part and NLP and lang detection
...
def get_OCR_single(image, zone):
    ...
    text: Text = pytesseract.image_to_string(crop, lang="fra+eng", config="--psm 6")
...
def filter_zones(zones, minX, minY, maxX, maxY, xPadding=8, yPadding=3):
    ...
    for zone in zones:
        ...
        boundaryRect = Rect(
            int(minX - xPadding),
            int(minY - yPadding),
            int(maxX) - int(minX) + xPadding * 2,
            int(maxY) - int(minY) + yPadding * 2,
        )
...
def resolution_consistent_crop(
    image, minX, minY, maxX, maxY, filler_rgb=(255, 255, 255)
):
